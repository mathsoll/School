...
#buss.iloc[:, 0] = pd.to_datetime(buss.iloc[:, 0])
buss[['dag','måned','år',]] = buss.Dato.str.split("/", expand = True)

#Converting date to a format that LinearRegression accepts

#passasjerersum = buss.set_index('dag').groupby(pd.Grouper(freq='D'))['Passasjerer_Ombord'].sum().reset_index()
...
#antall.iloc[:, 0] = antall.iloc[:, 0].map(dt.datetime.toordinal)
#antall.set_index('Dato',inplace=True)
